...
@testset "Opex transmission" begin
    ...
    case[:transmission][1].Modes[1] = RefStatic("Transline", Power, t_cap, t_loss, t_opex_var, t_opex_fixed, 2, Dict("" => EmptyData()))
    ...
    # Cange transmission mode parameters
    t_opex_var = FixedProfile((fuel_opex_diff - marginal_diff) * gen_fuel_coeff)
    case[:transmission][1].Modes[1] = RefStatic("Transline", Power, t_cap, t_loss, t_opex_var, t_opex_fixed, 1, Dict("" => EmptyData()))
