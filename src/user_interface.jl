...
function read_data()
    ...
    transmission = [Transmission(areas[1], areas[2], [OverheadLine_50MW]),
                    Transmission(areas[3], areas[1], [OverheadLine_50MW]),
                    Transmission(areas[2], areas[3], [OverheadLine_50MW]),
                    Transmission(areas[3], areas[4], [OverheadLine_50MW]),
                    Transmission(areas[1], areas[5], [OverheadLine_50MW]),
                    Transmission(areas[2], areas[5], [OverheadLine_50MW]),
                    Transmission(areas[5], areas[6], [OverheadLine_50MW]),
                    Transmission(areas[6], areas[7], [OverheadLine_50MW]),
...
function get_sub_system_data(i,𝒫₀, 𝒫ᵉᵐ₀, products;
    ...
    nodes = [
            ...
            EMB.RefGeneration(j+4, FixedProfile(25*gen_scale), FixedProfile(5.5*mc_scale), FixedProfile(10), Dict(NG => 2), Dict(Power => 1, CO2 => 1), 𝒫ᵉᵐ₀, 0.9, Dict(""=>EMB.EmptyData())),  
