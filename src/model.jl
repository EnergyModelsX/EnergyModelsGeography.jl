...
function EMB.variables_flow(m, ℒᵗʳᵃⁿˢ::Vector{Transmission}, 𝒳ᵛᵉᶜ, 𝒫, 𝒯,  modeltype::EnergyModel)
    ...
    for p_sub in EMB.res_types_vec(𝒫)
...
The default methods are empty and serve as dispatch hooks for extension packages that add
...
The default method is empty and serves as a dispatch hook for extension packages.
"""
